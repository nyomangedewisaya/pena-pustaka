...
<div x-data="{ open: false }" class="min-h-screen w-full flex items-center justify-center p-4 md:p-6 overflow-hidden relative">
    ...
    <div class="bg-white w-full max-w-5xl rounded-2xl shadow-2xl shadow-blue-900/10 overflow-hidden flex flex-col max-h-[90vh] overflow-y-auto scroll-smooth relative z-10">

        <div class="bg-white/90 backdrop-blur-md sticky top-0 z-20 px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                <span class="px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide {{ $book->stock > 0 ? 'bg-green-100 text-green-700 border border-green-200' : 'bg-red-100 text-red-700 border border-red-200' }}">
                    {{ $book->stock > 0 ? 'Tersedia' : 'Habis' }}
                </span>
                <span class="px-3 py-1 bg-blue-50 text-blue-700 border border-blue-100 rounded-full text-xs font-bold uppercase tracking-wide">
        ...
        <div class="p-6 md:p-10">
            <div class="flex flex-col md:flex-row gap-8 md:gap-12">
                ...
                <div class="shrink-0 mx-auto md:mx-0 w-48 md:w-64 flex flex-col gap-6">
                    <div class="aspect-2/3 rounded-xl overflow-hidden shadow-xl shadow-gray-200 border border-gray-100 relative group">
                        @if ($book->cover)
                    ...
                    <div class="text-center md:hidden">
                        ...
                        <p class="text-sm text-gray-500 mt-1 font-medium">{{ number_format($avgRating, 1) }} / 5.0</p>
                    </div>
                </div>
                ...
                <div class="flex-1 flex flex-col h-full">
                    <div class="mb-6">
                        ...
                        <p class="text-lg text-gray-600 font-medium">Penulis: <span class="text-blue-600">{{ $book->author }}</span></p>
                    </div>

                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 bg-gray-50 p-5 rounded-2xl border border-gray-100">
                        <div>
                            <p class="text-[10px] text-gray-400 uppercase font-bold tracking-wider mb-1">Penerbit</p>
                            <p class="text-sm font-bold text-gray-800 truncate" title="{{ $book->publisher }}">{{ $book->publisher }}</p>
                        </div>
                        <div class="md:ml-8">
                        ...
                        <div>
                            ...
                            <p class="text-sm font-bold text-gray-800 {{ $book->stock < 3 ? 'text-red-600' : '' }}">{{ $book->stock }} Eks</p>
                        </div>
                        <div>
                    ...
                    @if ($book->stock > 0)
                        <div class="mb-8">
                            <button @click="open = true"
                                class="w-full md:w-auto px-8 py-3.5 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-lg shadow-blue-600/20 active:scale-95 transition-all duration-200 flex items-center justify-center gap-3">
                                <i class="bi bi-journal-plus text-lg"></i>
                                <span>Ajukan Peminjaman</span>
                            </button>
                            <p class="text-xs text-gray-400 mt-2 ml-1">* Pastikan Anda sudah login untuk meminjam buku.</p>
                        </div>
                    @else
                        <div class="mb-8 p-4 bg-gray-100 rounded-xl border border-gray-200 text-center">
                            <span class="text-gray-500 font-medium flex items-center justify-center gap-2">
                                <i class="bi bi-exclamation-circle"></i> Maaf stok buku saat ini sedang kosong.
                            </span>
                        </div>
                    @endif
        ...
        <div class="p-6 md:p-10 bg-white pb-20">
            <div class="flex items-center justify-between mb-8">
                <h3 class="text-xl font-bold text-gray-800 flex items-center gap-2">
                    ...
                    <span class="px-2 py-0.5 bg-gray-100 text-gray-600 rounded text-xs ml-2 border border-gray-200">{{ $book->reviews->count() }}</span>
                </h3>
            </div>
            ...
            <div class="space-y-6">
                @forelse ($book->reviews as $review)
                    <div class="flex gap-4 pb-6 border-b border-gray-100 last:border-0 last:pb-0">
                        <div class="shrink-0 w-10 h-10 md:w-12 md:h-12 rounded-full bg-linear-to-br from-blue-500 to-indigo-600 text-white flex items-center justify-center font-bold text-lg shadow-md ring-2 ring-white">
                        ...
                        <div class="flex-1">
                            ...
                            <div class="flex text-yellow-400 text-xs mb-2">
                                @for ($i = 1; $i <= 5; $i++)
                                    <i class="bi {{ $i <= $review->rating ? 'bi-star-fill' : 'bi-star text-gray-300' }}"></i>
                                @endfor
                            </div>
                ...
                @empty
                    <div class="text-center py-12 px-4 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
                        ...
                        <p class="text-sm text-gray-500">Jadilah yang pertama memberikan ulasan untuk buku ini!</p>
                    </div>
                @endforelse
            </div>
        </div>
    ...
    <div x-show="open" style="display: none;" x-on:keydown.escape.window="open = false" class="relative z-50" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            ...
            <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
                <div x-show="open" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-lg border border-gray-100" @click.away="open = false">
                    ...
                    <div class="px-4 py-6 sm:px-6">
                        <p class="text-sm text-gray-500 mb-6">Apakah Anda yakin ingin meminjam buku ini? Silakan cek detail di bawah ini sebelum melanjutkan.</p>
                        ...
                        <div class="flex gap-4 bg-blue-50/50 p-4 rounded-xl border border-blue-100 mb-6">
                            <div class="shrink-0 w-16 h-24 rounded-lg overflow-hidden shadow-sm">
                                ...
                                @else
                                    <div class="w-full h-full bg-gray-200 flex items-center justify-center text-gray-400"><i class="bi bi-book"></i></div>
                                @endif
                            </div>
                            <div>
                                <h4 class="font-bold text-gray-800 line-clamp-1">{{ $book->title }}</h4>
                                <p class="text-xs text-gray-500 mb-2">{{ $book->author }}</p>
                                <span class="px-2 py-0.5 bg-white text-blue-600 border border-blue-200 rounded text-[10px] font-bold uppercase">{{ $book->category->name }}</span>
                            </div>
                        </div>
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex justify-between items-center text-sm pt-3 border-t border-dashed border-gray-200">
                                <span class="text-gray-500">Wajib Kembali Sebelum</span>
                                <span class="font-bold text-blue-600">{{ now()->addDays(7)->format('d M Y') }}</span>
                            </div>
                        </div>
                    </div>

                    <div class="bg-gray-50 px-4 py-4 sm:flex sm:flex-row-reverse sm:px-6 border-t border-gray-100 gap-2">
                        <form action="{{ route('book.borrow') }}" method="POST" class="w-full sm:w-auto">
                            @csrf
                            <input type="hidden" name="book_id" value="{{ $book->id }}">
                            
                            <button type="submit" class="inline-flex w-full justify-center rounded-lg bg-blue-600 px-3 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-500 sm:w-auto transition-colors">
                                Ya, Pinjam Buku
                            </button>
                        </form>
                        
                        <button type="button" @click="open = false" class="mt-3 inline-flex w-full justify-center rounded-lg bg-white px-3 py-2.5 text-sm font-semibold text-gray-900 shadow-sm ring-1 ring-inset ring-gray-300 hover:bg-gray-50 sm:mt-0 sm:w-auto transition-colors">
                            Batal
                        </button>
                    </div>
                </div>
