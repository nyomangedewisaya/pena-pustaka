...
@section('title', 'Buku Pena Pusaka')
@section('content')

    <x-header title="Kelola Buku" icon="bi-book" link="/books/create" createLink="Tambah Buku"
        caption="Kelola semua daftar buku anda disini." countData="{{ $countData }}" />
        ...
        <form action="{{ route('books') }}" method="GET"
            ...
            <div class="flex flex-col md:flex-row items-center justify-end gap-4">
                ...
                <div class="relative w-full sm:w-72">
                    <i class="bi bi-collection absolute left-3 top-2 text-gray-400"></i>
                    <select name="category_id"
                        class="w-full bg-white rounded-xl border border-gray-300 outline-none pl-10 pr-8 px-4 py-1.5 appearance-none focus:ring-1 focus:ring-blue-500">
            ...
            <table class="w-full text-gray-700 text-left">
                <thead class="uppercase font-medium text-xs bg-gray-100 text-gray-500">
                    <tr>
                        ...
                        <th class="px-4 py-3 font-semibold text-nowrap">Cover</th>
                        <th class="px-4 py-3 font-semibold text-nowrap">Judul</th>
                        <th class="px-4 py-3 font-semibold text-nowrap">Pengarang</th>
                        <th class="px-4 py-3 font-semibold text-nowrap">Penerbit</th>
                        <th class="px-4 py-3 font-semibold text-nowrap">Stok</th>
                        <th class="px-4 py-3 font-semibold text-nowrap text-center">Review</th>
                        ...
                        <tr class="hover:bg-gray-50 duration-150">
                            ...
                            <td class="px-4 py-3 font-medium text-nowrap">
                                <img src="{{ asset('storage/' . $item->cover) }}" alt="Cover"
                                    class="h-14 w-10 object-cover rounded shadow-sm bg-gray-100">
                            </td>
                            <td class="px-4 py-3 font-semibold text-gray-700 text-nowrap">{{ $item->title }}</td>
                            <td class="px-4 py-3 font-medium text-gray-600 text-nowrap">{{ $item->author }}</td>
                            <td class="px-4 py-3 font-medium text-gray-600 text-nowrap">{{ $item->publisher }}</td>
                            <td class="px-4 py-3 font-medium text-nowrap">
                                <span
                                    class="rounded-full px-2 py-0.5 text-xs font-semibold {{ $item->stock == 0 ? 'bg-red-100 text-red-600 border border-red-200' : 'bg-blue-100 text-blue-600 border border-blue-200' }}">
                                    {{ $item->stock }}
                                </span>
                            </td>
                            <td class="px-4 py-3 font-medium text-nowrap">
                                <div class="flex flex-col items-center gap-1">
                                    <div class="flex items-center gap-1">
                                        <span
                                            class="text-gray-700">{{ number_format($item->reviews_avg_rating ?? 0, 1) }}</span>
                                        <i class="bi bi-star-fill text-xs text-yellow-500"></i>
                                    </div>
                                    <p class="text-xs bg-yellow-100 text-yellow-700 rounded-xl px-2 py-0.5"> {{ $item->reviews()->count() }}
                                        Ulasan</p>
                                </div>
                            </td>
                            <td class="px-4 py-3 font-medium text-nowrap">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    <button @click="openDataBook(@js($item))"
                        ...
                        <tr class="bg-gray-50">
                            <td colspan="9" class="px-4 py-10 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <p class="text-sm font-medium text-gray-600">Tidak ada data buku ditemukan atau
                                        kosong.</p>
                                </div>
                            </td>
                        </tr>
                ...
                <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                    <div x-show="deleteModal === true" x-transition:enter="ease-out duration-300"
                                ...
                                <div class="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left w-full">
                                    ...
                                    <div class="mt-3">
                                        <p class="text-sm text-gray-500">
                                            Apakah anda yakin ingin menghapus buku <span class="font-bold text-gray-900"
                                                x-text="dataBook.title"></span>?
                                        </p>
                                        <p
                        ...
                        <div class="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                            <form :action="dataBook.action" method="POST" class="w-full sm:w-auto sm:ml-3">
                                @csrf
                                @method('DELETE')
                                <button type="submit"
    ...
    <script>
        function alpineInit() {
            return {
                ...
                openDataBook(data) {
                    this.dataBook.id = data.id;
                    this.dataBook.title = data.title;
                    this.dataBook.action = `/books/${data.id}/delete`;
